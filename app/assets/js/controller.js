...
app.controller('Processos', function($scope, $http, UsuarioService, $rootScope, toaster, $stateParams){
	$scope.botao = true;
	$scope.botao_atualizar = true;
	...
	$scope.submitForm = function(valid){
		...
		if(valid){
			...
			$http.post(url+'identificacao', $scope.processo_identificacao).then(function(data){
				if(data.data.sucesso){
					toaster.pop('success', "Sucesso", data.data.mensagem, 5000);
				}
				else{
	...
	$scope.filtraProcessoIdentificacao = function(valid){
		...
		if(valid){
			//$scope.tabelaLink = [];
			$http.post(url+'filtra-identificacao', $scope.processo_identificacao).then(function(data){				
				$scope.tabelaProcIdentificacao = data.data.processo_identificacao;
				console.log('$scope.tabelaLink', $scope.tabelaProcIdentificacao);
			})
		}
	}
	if($stateParams.param == 0){
